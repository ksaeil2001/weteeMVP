...
// 임시 저장 키
const DRAFT_STORAGE_KEY = 'wetee_lesson_record_draft_new';

// Inner component that uses useSearchParams
function LessonRecordNewPageContent() {
  ...
  useEffect(() => {
    if (!scheduleId && !authLoading) {
      setScheduleError('수업 ID(scheduleId)가 필요합니다. URL에 ?scheduleId=... 파라미터를 추가해주세요.');
      setScheduleLoading(false);
    }
  }, [scheduleId, authLoading]);

  // 일정 정보 로드
  useEffect(() => {
    if (!scheduleId) return;

    // scheduleId를 지역 변수에 저장하여 타입을 확정
    const currentScheduleId: string = scheduleId;

    async function loadSchedule() {
      try {
        setScheduleLoading(true);
        setScheduleError(null);
        const data = await fetchScheduleById(currentScheduleId);
        setSchedule(data);
  ...
  const validate = useCallback((): string | null => {
    ...
    // 숙제 입력했는데 기한 없으면 경고 (선택적)
    if (homework.trim() && !homeworkDueDate) {
      // 경고만 표시, 저장은 허용
      console.warn('숙제를 입력했지만 제출 기한이 설정되지 않았습니다.');
    }

    return null;
  }, [content, progressEntries, homework, homeworkDueDate]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (statusParts.length > 0) {
        combinedFeedback = statusParts.join(' / ');
      }
  ...
  function handleCancel() {
    if (content.trim() || homework.trim() || studentNotes.trim()) {
  ...
  // 학생 이름 표시
  const studentNames = schedule?.studentNames?.join(', ') || '학생 정보 없음';
        ...
        <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
          ...
          <div className="space-y-2">
            ...
            <div>
              ...
              <p className="font-semibold text-gray-900">{studentNames}</p>
            </div>
            {schedule?.title && (
              <div>
...
export default function LessonRecordNewPage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-gray-50 flex items-center justify-center">
