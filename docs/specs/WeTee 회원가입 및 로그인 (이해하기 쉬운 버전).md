...
flowchart TD
    ...
    A1 --> A2[기본 정보 입력<br/>이메일, 비밀번호, 이름, 전화번호]
    A2 --> A3[이메일 중복·형식 확인]
    A3 -->|유효함| A4[이메일 인증 코드 발송]
    A3 -->|문제 있음| A3E[에러 메시지 표시<br/>"이미 가입된 이메일입니다" 등]

    A4 --> A5[선생님: 메일에서 인증 코드 확인 후 입력]
    A5 -->|코드 일치| A6[계정 활성화 (선생님 계정 생성 완료)]
    A5 -->|코드 불일치/만료| A5E[에러 안내 및 재시도]

    A6 --> A7[선생님 대시보드 첫 진입]
    A7 --> A8["학생/학부모 초대 코드 만들기" 버튼 클릭]
    A8 --> A9[초대 코드 생성<br/>유효 기간·사용 횟수 설정]
    A9 --> A10[생성된 코드 학생/학부모에게 전달]

    %% 학생 가입
    B0[학생: 앱/웹 접속] --> B1["학생으로 가입하기" 선택]
    B1 --> B2[선생님에게 받은 초대 코드 입력]
    B2 --> B3[초대 코드 유효성 확인]
    B3 -->|유효함| B4[학생 기본 정보 입력<br/>이메일, 비밀번호, 이름, 학교/학년]
    B3 -->|유효하지 않음| B3E[에러 안내<br/>"코드가 잘못되었거나 만료되었습니다"]
    ...
    B7 --> B8[학생 대시보드 진입<br/>선생님·과외 그룹 자동 연결]

    %% 학부모 가입 (학생과 거의 동일)
    C0[학부모: 앱/웹 접속] --> C1["학부모로 가입하기" 선택]
    C1 --> C2[초대 코드 입력]
    C2 --> C3[초대 코드 유효성 확인<br/>(학부모용인지, 자녀와 연결되는지)]
    C3 -->|유효함| C4[학부모 정보 입력<br/>이메일, 비밀번호, 이름, 자녀와 관계]
    ...
    C7 --> C8[학부모 대시보드 진입<br/>자녀·선생님·과외 그룹 정보 연결]

    %% 비밀번호 재설정 공통 플로우
    D0[로그인 화면] --> D1["비밀번호를 잊으셨나요?" 클릭]
    D1 --> D2[이메일 입력]
